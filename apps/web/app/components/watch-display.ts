  ...
  const suffix =
    rule.metric === "temperature"
      ...
      : rule.metric === "precipitation_probability"
        ? "%"
        : ` ${rule.unit ?? "mph"}`;
  // The notice is a setting, visible when editing. On a two-across card it
  // pushed the threshold — the only part worth reading — off the end.
  const notice = "";
...
export function describeWatch(w: WatchRow, current?: number): WatchDisplay {
  ...
  if (w.source === "music" || w.source === "screen") {
    ...
    return {
      ...
      value: `${days}d`,
      delta,
      caption: known ? `Last ${thing} ${days}d ago` : `Watching ${days}d`,
